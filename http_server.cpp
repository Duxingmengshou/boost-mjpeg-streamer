...
void http_server::run_service_io()
{
	io_service_sp->run();
}
...
void http_server::handle_device_request(socket_shared_ptr socket, const boost::beast::http::request<boost::beast::http::string_body>& req) {
	...
	for (auto c : frames)
	{
		res.body() += "<body><h2>" + c.first + "</h2></body>";
	}
...
void http_server::handle_not_found(socket_shared_ptr socket, const boost::beast::http::request<boost::beast::http::string_body>& req) {
...
void http_server::handle_sn_request(socket_shared_ptr socket, const boost::beast::http::request<boost::beast::http::string_body>& req, std::string sn) {
	...
	while (true) {
		...
		boost::beast::http::response<boost::beast::http::vector_body<char>> res{ std::piecewise_construct,
			std::make_tuple(std::move(local_buffer)),
